...
;(function (globalScope) {
  ...
  P.normalize=function (){
    ...
    for (var i=0;i<x.array.length;++i){
      ...
      if (e[0]!==0&&(e[1]===0||e[1]===null||e[1]===undefined||e[2]===null||e[2]===undefined)){
        x.array.splice(i,1);
        --i;
        continue;
      }
  ...
  P.toJSON=function (){
    if (ExpantaNum.serializeMode==ExpantaNum.JSON){
      var a=[];
      for (var i=0;i<this.array.length;++i) a.push([this.array[i][0],this.array[i][1],this.array[i][2]]);
      return {
        array:a,
        layer:this.layer,
        sign:this.sign
  ...
  Q.fromArray=function (input1,input2,input3){
    ...
    if (!array.length) x.array=[[0,0,0]];
      ...
      x.array=[];
      for (i=0;i<array.length;i++){
        if (!(array[i] instanceof Array)||typeof array[i][0]!="number"||typeof array[i][1]!="number"||typeof array[i][2]!="number") throw Error(invalidArgument+"Expected Array of 3 Numbers");
        x.array.push([array[i][0],array[i][1],array[i][2]]);
      }
    }else throw Error(invalidArgument+"Expected Array of Number or Array of 3 Numbers");
  ...
  P.operator=function (i,i2,value){
    if (value===undefined) return this.getOperator(i,i2);
    else this.setOperator(i,value);
  };
  P.clone=function (){
    var temp=new ExpantaNum();
    var array=[];
    for (var i=0;i<this.array.length;++i) array.push([this.array[i][0],this.array[i][1],this.array[i][2]]);
    temp.array=array;
    temp.sign=this.sign;
  ...
  function clone(obj) {
    ...
    function ExpantaNum(input,input2) {
      ...
      if (typeof input=="number"&&!(input2 instanceof Array)){
        ...
        temp=[];
        for (var i=0;i<input.array.length;++i) temp.push([input.array[i][0],input.array[i][1],input.array[i][2]]);
        temp2=input.sign;
        temp3=input.layer;
